...
import {PLATFORM_NAME_TVOS, PLATFORM_NAME_IOS, PLATFORM_NAME_VISIONOS} from './desired-caps';
...
/**
 * Check if platform name is the visionOS one.
 *
 * @param {string|null|undefined} platformName
 * @returns {boolean}
 */
export function isVisionOs(platformName) {
  return _.toLower(platformName ?? '') === _.toLower(PLATFORM_NAME_VISIONOS);
}
...
export function normalizePlatformName(platformName) {
  ...
  if (isVisionOs(platformName)) {
    return PLATFORM_NAME_VISIONOS;
  }
